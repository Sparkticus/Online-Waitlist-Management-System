...
public class WW_Home extends HttpServlet {
    ...
    try {
      ...
      if (submit==null) {
        ...
        if (processLogin(session,req, out,con)>0){
          out.println("You've successfully logged in!<br>");
        } else {
    ...
    throws SQLException
  {
     ...
     out.println("<form action=/walter/servlet/WW_CreateAccount><button type=submit>Sign up!</button></form></tr></td>");
     out.println("</table>");
   }
